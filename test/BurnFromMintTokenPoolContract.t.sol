...
import {BurnFromMintTokenPoolContract} from "src/BurnFromMintTokenPoolContract.sol";
...
contract TestBurnFromMintTokenPoolContract is Test {
    uint256 private PRIVATE_KEY;
    address public deployer;

    BurnFromMintTokenPoolContract bsc_c;
    Erc20Token bsc_myToken;

    BurnFromMintTokenPoolContract eth_c;
    ...
    function setUp() public {
        ...
        bscNetworkDetails = bscCcipLocalSimulatorFork.getNetworkDetails(block.chainid);
        ...
        // pool设置
        bsc_c = new BurnFromMintTokenPoolContract(
        ...
        // token注册
        RegistryModuleOwnerCustom(bscNetworkDetails.registryModuleOwnerCustomAddress).registerAdminViaGetCCIPAdmin(address(bsc_myToken));
        ITokenAdminRegistry(bscNetworkDetails.tokenAdminRegistryAddress).acceptAdminRole(address(bsc_myToken));
        ITokenAdminRegistry(bscNetworkDetails.tokenAdminRegistryAddress).setPool(address(bsc_myToken), address(bsc_c));
        ...
        ethNetworkDetails = ethCcipLocalSimulatorFork.getNetworkDetails(block.chainid);
        ...
        // pool设置
        eth_c = new BurnFromMintTokenPoolContract(
       ...
       // token注册
        RegistryModuleOwnerCustom(ethNetworkDetails.registryModuleOwnerCustomAddress).registerAdminViaGetCCIPAdmin(address(eth_myToken));
        ITokenAdminRegistry(ethNetworkDetails.tokenAdminRegistryAddress).acceptAdminRole(address(eth_myToken));
        ITokenAdminRegistry(ethNetworkDetails.tokenAdminRegistryAddress).setPool(address(eth_myToken), address(eth_c));
        ...
        // Configure BSC Pool
        vm.selectFork(bscFork);
        TokenPool.ChainUpdate[] memory bscChainsToAdd = new TokenPool.ChainUpdate[](1);
        ...
        // Configure ETH Pool
        vm.selectFork(ethFork);
        TokenPool.ChainUpdate[] memory ethChainsToAdd = new TokenPool.ChainUpdate[](1);
    ...
    function testDeploy() public {
        vm.selectFork(bscFork);
        assertEq(bsc_myToken.MINTER_ROLE(), keccak256("MINTER_ROLE"));
        ...
        // 使用 CCIP 发送消息
        IRouterClient(bscNetworkDetails.routerAddress).ccipSend{value: 1 ether}(ethNetworkDetails.chainSelector, message);
