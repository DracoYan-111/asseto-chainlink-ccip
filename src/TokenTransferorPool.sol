...
contract TokenTransferorPool is BurnFromMintTokenPool {
  ...
  {
    if (!isSupportedChain(_destinationChainSelector)) revert DestinationChainNotAllowlisted(_destinationChainSelector);

    // Get the fee required to send the message
    uint256 fees = getRouterFee(_destinationChainSelector, _receiver, _token, _amount);
    ...
    messageId = IRouterClient(getRouter()).ccipSend{value: fees}(_destinationChainSelector,_buildCCIPMessage(_receiver, _token, _amount, address(0)) );
    ...
    function getRouterFee(
      ...
      // Get the fee required to send the message
      return IRouterClient(getRouter()).getFee(_destinationChainSelector, _buildCCIPMessage(_receiver, _token, _amount, address(0)));
    }
